...
static unsigned long shrink_scan(struct shrinker *shrink, struct shrink_control *sc)
{
	...
	list_for_each_safe(pos, tmp, first_sample)
	{
		if (nr_to_scan > freed)
		{
			...
			mempool_free(list_entry(pos, struct task_sample, list), pool);
			freed++;
		}
		else
		{
...
void save_sample(void){
	...
	s = (struct task_sample*)mempool_alloc(pool, GFP_KERNEL);
	if (s == NULL)
	{
...
static int exo02_init(void)
{
	...
	task_sample_slab = KMEM_CACHE(task_sample,  GFP_KERNEL);
	pool = mempool_create(MEMPOOLSIZE, mempool_alloc_slab, mempool_free_slab, task_sample_slab); 
